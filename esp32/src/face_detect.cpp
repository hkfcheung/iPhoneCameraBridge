...
// Downscaled dimensions: VGA 640x480 / 4 = 160x120
...
static const int   kMinSkinCluster    = 200;   // minimum skin pixels in a region
...
void faceDetectLoop() {
    ...
    bool ok = jpg2rgb565(fb->buf, fb->len, (uint8_t *)curBuf, JPG_SCALE_4X);
    esp_camera_fb_return(fb);

    if (!ok) {
        Serial.println("[FACE] jpg2rgb565 failed");
        return;
    }
    ...
    // Step 1: Detect motion + skin color in one pass
    int motionPixels = 0;
    int skinPixels = 0;
    ...
    if (motionFrac < kMotionFraction) {
        if (confirmCount > 0) {
            Serial.printf("[FACE] no motion, reset confirm=%d\n", confirmCount);
            confirmCount = 0;
        }
        return;
    }

    if (skinPixels < kMinSkinCluster || (float)skinPixels / kPixels < kSkinFraction) {
        if (confirmCount > 0) {
            Serial.printf("[FACE] motion but no skin (motion=%.1f%%, skin=%d), reset\n",
                          motionFrac * 100, skinPixels);
            confirmCount = 0;
        }
        return;
    }
    ...
    confirmCount++;
    Serial.printf("[FACE] detected (motion=%.1f%%, skin=%d, confirm=%d/%d)\n",
                  motionFrac * 100, skinPixels, confirmCount, FACE_CONFIRM_COUNT);
